...
if dev:
    model_deployment_crn=metrics.dev_model_deployment_crn # update modelDeploymentCrn
...
for i in range(len(score_x)):
    if model_access_key is not None:
        output = models.call_model(model_access_key, {"petal_length": score_x[i][0]}, api_key=user_api_key)["response"]
    else:
...
data = metrics.read_metrics(model_deployment_crn=model_deployment_crn,
...
for i in range(n):
    ...
    metrics.track_delayed_metrics({"actual_result":ground_truth}, uuids[i], dev=dev)

    absolute_error = abs(ground_truth - predictions[i])
